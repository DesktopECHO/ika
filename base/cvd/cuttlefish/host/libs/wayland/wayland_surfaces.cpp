...
bool Surfaces::HandleSurfaceDmabufFrame(
    ...
    uint32_t frame_fourcc_format, int dmabuf_fd, uint32_t frame_offset,
    uint32_t frame_stride_bytes, uint32_t modifier_hi, uint32_t modifier_lo) {
  std::unique_lock<std::mutex> lock(callback_mutex_);

  if (dmabuf_callback_) {
    return (dmabuf_callback_.value())(
        display_number, frame_width, frame_height, frame_fourcc_format,
        dmabuf_fd, frame_offset, frame_stride_bytes, modifier_hi, modifier_lo);
  }
  return false;
}
