...
| [`bison.patch`](bison.patch) | source root | Points libpcap, one-true-awk, iproute2, and Mesa Bison genrules at the checked-in build-tools data directory so parser generation works with project-managed prebuilts. |
...
| [`external-mesa3d.patch`](external-mesa3d.patch) | `external/mesa3d` | Keeps gfxstream DRM-format-modifier image queries on the host pass-through path when the host advertises modifier support, and applies Android's imported-AHardwareBuffer layout-preservation workaround to sync2 pipeline barriers so Chromium/ANGLE Vulkan surfaces are not stripped or discarded into black content. |
...
| [`device-google-cuttlefish.patch`](device-google-cuttlefish.patch) | `device/google/cuttlefish` | Adds the Cuttlefish device foundation for desktop products: desktop product entries, ARMv8.2-A minimum ARM64 config, PC feature declarations, batteryless health behavior, graphics property plumbing, desktop boot defaults, ADB/provisioning setup, x86_64/native-bridge product config, and ROM-first AVB key lookup. |
