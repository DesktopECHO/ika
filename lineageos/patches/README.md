...
| [`external-erofs-utils-arm64-host-no-mt.patch`](external-erofs-utils-arm64-host-no-mt.patch) | `external/erofs-utils` | Builds native ARM64 host `mkfs.erofs` without `EROFS_MT_ENABLED` to avoid signing-time EROFS worker hangs, while x86 hosts keep the upstream multithreaded default. |
